...
function LocationInput({ value, onChange }) {
    ...
    return (
        ...
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search city..."
            className="w-full rounded-xl border-2 border-crave-ink px-3 py-2" />
        {results.length > 0 && (
            <ul className="absolute z-10 w-full rounded-xl border-2 border-crave-ink bg-crave-bone shadow-hard-sm">
            {results.map((r) => (
